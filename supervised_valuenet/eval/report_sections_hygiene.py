...
def sec_databudget(D):
    ...
    ledger = (
        ...
        "the only learning-rate sweep either side ever received went to "
        "the move-by-move planner, at 16\u00d716 with 8 robots \u2014 nine "
        "retrainings, three seeds by three learning rates, best-by-"
        "validation (fairness tab). Elsewhere the forward planner is "
        "single-seed, and only the subgoal planner's two single-draw "
        "headline rungs have been repeated under fresh seeds, so no "
        "small difference between the systems is defensible against "
        "seed variation. The ledger also "
