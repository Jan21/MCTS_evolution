...
Dynamic decomposition / factoring in AND–OR graphs

Search over goal-sets (compound subproblems)

“Split vs solve jointly” as an explicit OR decision

AO* / AND–OR best-first cost backup (min over OR, sum over AND)

Interaction / dependency graph for independence tests

Lazy decomposition (assume independence first)

Conflict-driven merging (“merge only the culprit edges”)

Conflict set extraction / culprit identification

Conditional independence via a shared context / constraint store

Resource-profile summaries returned by subproblem solvers

Coordination layer / global feasibility check (resource + mutex + ordering)

Progressive merging / component merging strategy

Conflict graph over segments (connected components define current decomposition)

Segment “signatures” / “footprints” (mover, required blockers, committed positions)

Mover–blocker coupling detection (segment moves a robot another segment needs fixed)

Helper-reuse detection (same helper required at multiple positions)

Relocation / connector penalties between required blocker positions

Treating infeasible combinations as hard constraints (∞ penalty)

Soft penalties vs hard conflicts distinction

MCTS over partial plans (AND node = partial plan DAG with open segments)

Contextual refinement (expand segments in the context of the current partial plan)

Action = choose segment to expand + choose subgoal to apply

Fixed segment traversal / precedence constraints as “time indices” / windows

Two-stage bandit: choose segment, then choose subgoal

Progressive widening for large action spaces (segments and/or subgoals)

Rollout-based coupling discovery (detect conflicts during simulation)

Learning couplings by persisting conflict edges at visited nodes

Backing up complete sampled combinations (option tuples), not “best per segment”

Pairwise no-good learning / no-good tables for incompatible option pairs

Context-specific no-goods (keyed by a context signature)

Backjumping to the decision that introduced a conflicting commitment

“Repair then continue rollout” (resample at culprit choice)

Treat fixed plan as a constraint context (conditioning values on fixed commitments)

Commitment schema: BlockReq / EndPosReq / ProtectReq

Lazy “protect intervals” (don’t-move commitments added only when needed)

Disjunctive support commitments (SupportReq with multiple (helper,pos) choices)

Commitment strength levels (hard vs soft)

Conflict detection via overlapping protected intervals / incompatible endpos

Transposition tables (graph search / graph MCTS)

Segment-level transpositions (cache segment subproblems separately from plans)

Context-signature keys for caching (frozen robots, reserved blockers)

Relaxation cache for measure_path_length (context-independent memoization)

Reusing cached subgoal proposals as a superset + filtering by context

Using cached stats as priors / pseudo-count initialization across contexts

Cache hit-rate / reuse diagnostics

Anytime evaluation curves (best-so-far solution length vs time/rollouts)

AUC of solution quality over time as a headline metric

Time-to-first-solution metric

Success rate under budget metric

Throughput metrics (rollouts/sec, expansions/sec)

Coupling diagnostics (conflict rate, component sizes, wasted optimism)



# Algorithm Ideas

1. **Contextual Plan-Refinement Graph MCTS**

* **State/node:** full partial-plan DAG (open segments + commitments) with a **transposition table for partial plans** (graph MCTS).
* **Action:** two-stage bandit: pick **which segment to expand**, then pick a **subgoal** (progressive widening on both).
* **Evaluation:** exact-tail + relaxed `measure_path_length` sums + **context penalties** derived from commitments (helper reuse, mover–blocker).
* **Reuse:** cache relaxed distances globally; reuse plan nodes via transpositions so different derivations share stats.

2. **Lazy-Coupling MCTS with Conflict Graph + Progressive Merging**

* **State/node:** partial plan + a **conflict graph** over segments; connected components define current “decomposition.”
* **Action:** either (a) refine a segment normally, or (b) **merge a conflicted component** and refine it jointly (split-vs-joint as OR decision).
* **Rollout:** assume independence initially; during simulation run **coupling detection** (helper reuse, mover–blocker) and add conflict edges.
* **Learning:** persist conflict edges (and/or merge decisions) at visited nodes so future search focuses on coupled components.
* **Value:** add **relocation/connector penalties** within conflicted components; treat true impossibilities as hard conflicts.

3. **Constraint-Context MCTS with Deep No-Good Learning**

* **State/node:** partial plan with a **constraint store/context** built from commitments (BlockReq/ProtectReq/SupportReq).
* **Action:** refine a segment by proposing subgoals; each subgoal adds commitments (possibly disjunctive first, then commits lazily).
* **Conflict handling:** when a conflict occurs, **backjump** to the **decision that introduced** the conflicting commitment and record a **context-specific no-good** (or pairwise no-good).
* **Effect:** future traversals avoid the bad deep choice without penalizing the top-level template that created the segment.
* **Value:** primarily heuristic, but conditioned on the evolving context; hard conflicts get pruned/∞ penalty.

4. **Segment-Node MCTS with Context-Signature Transpositions**

* **State/node:** partial plan points to shared **segment subproblem nodes** via a **segment transposition table** keyed by (robot, goal, frozen/reserved signature).
* **Action:** at plan level, choose segment to expand; at segment-node level, choose which subgoal option to apply (both with UCB + progressive widening).
* **Reuse:** segment nodes cache proposed subgoals + their footprints and accumulate MCTS stats across many plans; plan nodes learn scheduling.
* **Coupling:** when footprints clash in a plan context, apply penalties and optionally add protect commitments; record pairwise penalties between segment options.
* **Outcome:** strong systematic reuse of “robot a → p” knowledge without assuming full independence.

5. **Nested (Two-Stage) MCTS: Skeleton First, Realization Second**

* **Outer MCTS:** searches over **plan skeletons** (high-level subgoals/support assignments; split-vs-joint decisions allowed).
* **Inner search/MCTS:** given a skeleton, attempts to realize relaxed segments under commitments using contextual refinement; returns cost + detected couplings/conflicts.
* **Feedback loop:** outer node values are based on inner success/cost; inner conflicts generate **conflict sets/no-goods** that constrain future skeleton choices.
* **Caching:** use transposition tables at both levels (skeleton signatures; segment signatures) and reuse relaxed distance computations.
* **Benefit:** clean separation of “choose helpful blockers/subgoals” from “make them actually compatible,” with systematic learning at both levels.
