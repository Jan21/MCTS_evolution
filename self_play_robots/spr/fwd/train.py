...
hygiene PROBLEM.md 4.4 demands and `train_on_records` does not have:
  ...
  * an optional exact ANCHOR corpus (`move_planner/data/moves*.jsonl`, the
    supervised forward records) mixed in, so the loop cannot drift off the
    supervised distribution -- with a HARD DROP of any record on a test/bench
    board (`nn.benchmark.SPLITS['test']`, which contains bench450's 2400-2549).
...
def main(argv=None):
    ...
    # BEFORE the first forward-stack import: `train.encode.ENV_DIR` is resolved at
    # import time, and a mixed buffer needs ONE board dir (populated below).
    orig_env_dir = Path(os.environ.get("RR_ENV_DIR") or (SV / "environments"))
    ...
    from nn.benchmark import SPLITS
    ...
    for path in a.records:
        ...
        sp += r
    anchor = []
    banned = set(SPLITS["test"])                 # 112-127 + 2400-2999 (bench lives here)
    n_banned = 0
