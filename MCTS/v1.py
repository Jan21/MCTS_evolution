from __future__ import annotations

import math
import time
from dataclasses import dataclass, field
from pathlib import Path

import networkx as nx
from omegaconf import OmegaConf

from GridEnv import GridEnv, Robot_at, State
from partial_plan import PartialPlan
from evaluate_plan import evaluate_plan
from MCTS.base import MCTS, SolveResult, PlanEntry, PlanStats
from mcts_tracer import MCTSTracer, TraceEvent, _serialize_plan


# ---------------------------------------------------------------------------
# MCTS tree node
# ---------------------------------------------------------------------------

@dataclass
class MCTSNode:
    plan: PartialPlan
    parent: MCTSNode | None
    action: tuple | None
    children: list[MCTSNode] = field(default_factory=list)
    visit_count: int = 0
    total_cost: float = 0.0
    best_cost: float = float("inf")

    @property
    def avg_cost(self) -> float:
        if self.visit_count == 0:
            return float("inf")
        return self.total_cost / self.visit_count


# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

def _counter():
    n = 0
    while True:
        yield n
        n += 1


def _copy_plan(plan: PartialPlan) -> PartialPlan:
    new = PartialPlan()
    new.g = plan.g.copy()
    return new


def _find_sibling_support_pos(plan: PartialPlan, bottleneck_id: str):
    g = plan.g
    for parent in g.predecessors(bottleneck_id):
        if g.nodes[parent].get("ntype") == "subgoal":
            for child in g.successors(parent):
                if g.nodes[child].get("ntype") == "support":
                    return g.nodes[child].get("pos")
    return None


def _find_sibling_support_robot(plan: PartialPlan, bottleneck_id: str):
    g = plan.g
    for parent in g.predecessors(bottleneck_id):
        if g.nodes[parent].get("ntype") == "subgoal":
            for child in g.successors(parent):
                if g.nodes[child].get("ntype") == "support":
                    return g.nodes[child].get("robot")
    return None


def _get_start_pos(g, node_id: str):
    ntype = g.nodes[node_id]["ntype"]
    if ntype == "subgoal":
        for c in g.successors(node_id):
            if g.nodes[c]["ntype"] == "bottleneck":
                return g.nodes[c]["pos"]
    return g.nodes[node_id].get("pos")


def _build_initial_plan(state: State, grid_env: GridEnv) -> PartialPlan:
    plan = PartialPlan()
    plan.add_node("goal", "goal", pos=state.target)
    plan.add_node("leaf_target", "leaf",
                  pos=state.target_robot.position,
                  robot=state.target_robot)
    relaxed_cost = grid_env.compute_relaxed_shortest_path_length(
        state.target_robot.position, state.target)
    plan.add_edge("goal", "leaf_target", status="open", cost=relaxed_cost)
    return plan


def _build_segment_states(plan: PartialPlan, parent_id: str, state: State,
                          grid_env: GridEnv):
    """Return list of (segment_state, support_robot) for propose_subgoal_states.

    Usually returns a single pair, but for goal nodes with only dependent edges,
    returns one pair per possible support position.
    """
    g = plan.g
    parent_data = g.nodes[parent_id]
    parent_type = parent_data["ntype"]

    if parent_type == "goal":
        goal_pos = parent_data["pos"]
        results = []
        # Try independent path (no support needed)
        if (goal_pos, None) in grid_env._bottleneck_support_pairs_cache:
            results.append((State(target=goal_pos,
                                  target_robot=state.target_robot,
                                  helpers=state.helpers), None))
        # Also enumerate dependent support positions
        seen_support = set()
        for _, _, d in grid_env.G.in_edges(goal_pos, data=True):
            if "dependent" in d:
                sp = d["dependent"]
                if sp in seen_support:
                    continue
                seen_support.add(sp)
                for helper in state.helpers:
                    sr = Robot_at(position=sp, color=helper.color)
                    results.append((State(target=goal_pos,
                                         target_robot=state.target_robot,
                                         helpers=state.helpers), sr))
        return results

    if parent_type == "bottleneck":
        sp_robot = _find_sibling_support_robot(plan, parent_id)
        return [(State(target=parent_data["pos"],
                       target_robot=parent_data["robot"],
                       helpers=state.helpers), sp_robot)]

    if parent_type == "support":
        helper = parent_data["robot"]
        remaining = [h for h in state.helpers if h.color != helper.color]
        sp_pos = parent_data["pos"]
        results = []
        if (sp_pos, None) in grid_env._bottleneck_support_pairs_cache:
            results.append((State(target=sp_pos,
                                  target_robot=helper,
                                  helpers=remaining), None))
        # Also enumerate dependent support positions
        seen_support = set()
        for _, _, d in grid_env.G.in_edges(sp_pos, data=True):
            if "dependent" in d:
                dep_sp = d["dependent"]
                if dep_sp in seen_support:
                    continue
                seen_support.add(dep_sp)
                for h in remaining:
                    sr = Robot_at(position=dep_sp, color=h.color)
                    results.append((State(target=sp_pos,
                                         target_robot=helper,
                                         helpers=remaining), sr))
        return results

    return []


def _get_ancestor_positions(plan, node_id):
    """Collect positions of all ancestor nodes in the plan DAG."""
    g = plan.g
    positions = set()
    for anc in nx.ancestors(g, node_id):
        pos = g.nodes[anc].get("pos")
        if pos is not None:
            positions.add(pos)
    pos = g.nodes[node_id].get("pos")
    if pos is not None:
        positions.add(pos)
    return positions


def _get_candidates(plan, parent_id, state, grid_env):
    """Collect all (subgoal, score, parent_support_pos) candidates.

    parent_support_pos is the support position needed for the bn->parent edge
    (comes from the support_robot passed to propose_subgoal_states).
    For parents that don't need dependent edges, parent_support_pos is None.
    Filters out candidates whose bottleneck would create a cycle.
    """
    pairs = _build_segment_states(plan, parent_id, state, grid_env)
    if not pairs:
        return []
    ancestor_pos = _get_ancestor_positions(plan, parent_id)
    all_candidates = []
    seen = set()
    for seg_state, support_robot in pairs:
        cache_key = (seg_state.target,
                     support_robot.position if support_robot else None)
        if cache_key not in grid_env._bottleneck_support_pairs_cache:
            continue
        try:
            candidates = grid_env.propose_subgoal_states(
                seg_state, support_robot)
        except (TypeError, AssertionError):
            continue
        parent_sp = support_robot.position if support_robot else None
        for subgoal, score in candidates:
            if score is not None:
                bn_pos = subgoal.bottleneck.position
                if bn_pos in ancestor_pos:
                    continue
                key = (bn_pos,
                       subgoal.support.position,
                       subgoal.helper.color,
                       parent_sp)
                if key not in seen:
                    seen.add(key)
                    all_candidates.append((subgoal, score, parent_sp))
    return all_candidates


def _apply_subgoal(plan, parent_id, child_id, subgoal, grid_env, cnt,
                   parent_support_pos=None):
    """Apply a subgoal to refine the open edge (parent_id -> child_id).

    parent_support_pos: the support position needed for the bn->parent
        dependent edge. For bottleneck parents this comes from their sibling
        support; for goal/support parents with only dependent in-edges this
        comes from the support_robot used during candidate generation.
        None when the parent is reachable via independent edges.

    Returns new node IDs tuple or None if invalid.
    """
    g = plan.g
    parent_data = g.nodes[parent_id]
    n = next(cnt)

    sg_id = f"sg_{n}"
    bn_id = f"bn_{n}"
    sp_id = f"sp_{n}"
    leaf_id = f"leaf_{subgoal.helper.color}_{n}"

    exact_cost = grid_env.compute_exact_shortest_path_length(
        subgoal.bottleneck.position, parent_data["pos"], parent_support_pos)
    if exact_cost is None:
        return None

    # Remove old open edge
    g.remove_edge(parent_id, child_id)

    # Add nodes (parent_support_pos stored on subgoal for evaluate_plan)
    plan.add_node(sg_id, "subgoal", parent_support_pos=parent_support_pos)
    plan.add_node(bn_id, "bottleneck",
                  pos=subgoal.bottleneck.position,
                  robot=subgoal.bottleneck)
    plan.add_node(sp_id, "support",
                  pos=subgoal.support.position,
                  robot=subgoal.support)
    plan.add_node(leaf_id, "leaf",
                  pos=subgoal.helper.position,
                  robot=subgoal.helper)

    # parent -> subgoal (fixed)
    plan.add_edge(parent_id, sg_id, status="fixed", cost=exact_cost)

    # structural edges
    plan.add_edge(sg_id, bn_id, status="fixed", cost=None)
    plan.add_edge(sg_id, sp_id, status="fixed", cost=None)

    # bottleneck -> old child (open)
    child_pos = _get_start_pos(g, child_id)
    relaxed_bn = grid_env.compute_relaxed_shortest_path_length(
        child_pos, subgoal.bottleneck.position, subgoal.support.position)
    plan.add_edge(bn_id, child_id, status="open",
                  cost=relaxed_bn if relaxed_bn is not None else 999)

    # support -> helper leaf (open)
    relaxed_sp = grid_env.compute_relaxed_shortest_path_length(
        subgoal.helper.position, subgoal.support.position)
    plan.add_edge(sp_id, leaf_id, status="open",
                  cost=relaxed_sp if relaxed_sp is not None else 999)

    return (sg_id, bn_id, sp_id, leaf_id)


def _try_close_edge(plan, parent_id, child_id, grid_env):
    """Try to close an open edge directly (without subgoal insertion).

    For bottleneck parents, also tries the dependent path through the
    sibling support position.

    Returns True if the edge was closed, False otherwise.
    """
    g = plan.g
    parent_data = g.nodes[parent_id]
    child_pos = _get_start_pos(g, child_id)
    parent_pos = parent_data["pos"]

    # Try independent path first
    exact = grid_env.compute_exact_shortest_path_length(
        child_pos, parent_pos, None)
    if exact is not None:
        g.edges[parent_id, child_id]["status"] = "fixed"
        g.edges[parent_id, child_id]["cost"] = exact
        return True

    # For bottleneck parents, try dependent path through sibling support
    if parent_data["ntype"] == "bottleneck":
        sp_pos = _find_sibling_support_pos(plan, parent_id)
        if sp_pos is not None:
            exact = grid_env.compute_exact_shortest_path_length(
                child_pos, parent_pos, sp_pos)
            if exact is not None:
                g.edges[parent_id, child_id]["status"] = "fixed"
                g.edges[parent_id, child_id]["cost"] = exact
                return True

    return False


def _can_close_edge(plan, parent_id, child_id, grid_env):
    """Read-only check: can this open edge be closed directly?

    Returns (True, exact_cost) if closeable, (False, None) otherwise.
    Does NOT mutate the plan.
    """
    g = plan.g
    parent_data = g.nodes[parent_id]
    child_pos = _get_start_pos(g, child_id)
    parent_pos = parent_data["pos"]

    exact = grid_env.compute_exact_shortest_path_length(
        child_pos, parent_pos, None)
    if exact is not None:
        return True, exact

    if parent_data["ntype"] == "bottleneck":
        sp_pos = _find_sibling_support_pos(plan, parent_id)
        if sp_pos is not None:
            exact = grid_env.compute_exact_shortest_path_length(
                child_pos, parent_pos, sp_pos)
            if exact is not None:
                return True, exact

    return False, None


def _should_close_edge(plan, parent_id, child_id, grid_env, state):
    """Decide if an open edge should be permanently closed.

    Close only when provably optimal: exact cost equals relaxed cost
    (no decomposition can improve it), or no candidate subgoals exist.
    """
    can, exact = _can_close_edge(plan, parent_id, child_id, grid_env)
    if not can:
        return False
    relaxed = plan.g.edges[parent_id, child_id]["cost"]
    if exact == relaxed:
        return True  # no decomposition can beat this
    candidates = _get_candidates(plan, parent_id, state, grid_env)
    return len(candidates) == 0


def _try_complete_plan(plan, grid_env):
    """Speculatively close all open edges on a copy.

    Returns (completed_plan, cost) if all edges close and evaluate succeeds,
    (None, None) otherwise.
    """
    trial = _copy_plan(plan)
    for pid, cid in list(trial.open_edges()):
        if not _try_close_edge(trial, pid, cid, grid_env):
            return None, None
    cost = evaluate_plan(trial, grid_env)
    if cost is not None:
        return trial, cost
    return None, None


# ---------------------------------------------------------------------------
# MCTS_V1 -- V1 Contextual Plan-Refinement
# ---------------------------------------------------------------------------

class MCTS_V1(MCTS):

    def __init__(self, cfg=None, trace: bool = True):
        if cfg is None:
            cfg = OmegaConf.load(
                Path(__file__).resolve().parent.parent / "conf" / "v1.yaml")
        self.cfg = cfg
        self._trace_enabled = trace
        self.tracer: MCTSTracer | None = None

    def solve(self, grid_env: GridEnv, state: State, debug_hook=None) -> SolveResult:
        """Run MCTS search.

        Args:
            debug_hook: Optional callback ``f(algo, iteration)`` invoked after
                each iteration when tracing is enabled. Use with
                ``tracer.dump_live()`` for live IDE debugging.
        """
        self._cnt = _counter()
        self._best_cost = float("inf")
        self._best_plan = None
        self._start_time = time.monotonic()
        self._all_plans = []
        self._iteration = 0
        self._node_count = 0
        self._rollout_count = 0

        # Set up tracer
        if self._trace_enabled:
            self.tracer = MCTSTracer()
            self.tracer.set_grid(grid_env, state)

        root_plan = _build_initial_plan(state, grid_env)
        root = MCTSNode(plan=root_plan, parent=None, action=None)

        if self.tracer:
            self.tracer.register_node(root)
            self.tracer.initial_plan = _serialize_plan(root_plan)

        for _ in range(self.cfg.max_iterations):
            self._iteration += 1
            node = self._select(root) # selects the node to expand

            if node.plan.is_complete():
                cost = self._eval_complete(node.plan, grid_env)
                self._backprop(node, cost)
                if self.tracer:
                    event = TraceEvent(
                        iteration=self._iteration,
                        phase="complete_node",
                        selected_node_id=self.tracer.get_node_id(node),
                        selection_path=self._trace_selection_path,
                        lcb_scores=self._trace_lcb_scores,
                        backprop_cost=cost,
                        best_cost=self._best_cost,
                        tree_node_count=self._node_count,
                        complete_plans_found=len(self._all_plans),
                        plan_snapshot=_serialize_plan(node.plan),
                    )
                    self.tracer.record_event(event)
                continue

            child = self._expand(node, grid_env, state)
            if child is None:
                self._backprop(node, self.cfg.penalty_cost)
                if self.tracer:
                    event = TraceEvent(
                        iteration=self._iteration,
                        phase="dead_end",
                        selected_node_id=self.tracer.get_node_id(node),
                        selection_path=self._trace_selection_path,
                        lcb_scores=self._trace_lcb_scores,
                        candidate_positions=self._trace_candidate_positions,
                        backprop_cost=self.cfg.penalty_cost,
                        best_cost=self._best_cost,
                        tree_node_count=self._node_count,
                        complete_plans_found=len(self._all_plans),
                        plan_snapshot=_serialize_plan(node.plan),
                    )
                    self.tracer.record_event(event)
                continue

            cost = self._rollout(child, grid_env, state)
            self._backprop(child, cost)

            if self.tracer:
                event = TraceEvent(
                    iteration=self._iteration,
                    phase="select_expand_rollout",
                    selected_node_id=self.tracer.get_node_id(node),
                    selection_path=self._trace_selection_path,
                    lcb_scores=self._trace_lcb_scores,
                    expanded_child_id=self.tracer.get_node_id(child),
                    target_edge=self._trace_target_edge,
                    candidates_count=self._trace_candidates_count,
                    candidate_positions=self._trace_candidate_positions,
                    action=self.tracer._serialize_action(child.action),
                    expansion_closed_edges=[list(e) for e in self._trace_expansion_closed],
                    rollout_steps=self._trace_rollout_steps,
                    rollout_cost=cost,
                    rollout_completed=self._trace_rollout_completed,
                    backprop_cost=cost,
                    best_cost=self._best_cost,
                    tree_node_count=self._node_count,
                    complete_plans_found=len(self._all_plans),
                    plan_snapshot=_serialize_plan(child.plan),
                )
                self.tracer.record_event(event)

            if debug_hook and self.tracer:
                debug_hook(self, self._iteration)

        best = self._best_plan if self._best_plan is not None else self._fallback(root, grid_env, state)
        return SolveResult(best_plan=best, all_plans=self._all_plans)

    # -- Selection (LCB for minimization) --

    def _select(self, node: MCTSNode) -> MCTSNode:
        """
        This is the selection phase of MCTS (Monte Carlo Tree Search). It traverses from the root down the tree to find a node to expand next.
        Step by step:

        Walk down the tree while the current node has children.
        At each level, check if the node still has untried actions (_can_expand) — if so, return it for expansion.
        Filter children to only viable ones whose plan cost doesn't exceed the best known solution (_best_cost) — this prunes unpromising branches.
        If no viable children exist, stop and return the current node.
        Among viable children, pick the one with the lowest LCB (Lower Confidence Bound) score — this balances exploitation (low cost) with exploration (uncertainty).
        Repeat until reaching a leaf or an expandable node.
        """
        self._trace_selection_path = []
        self._trace_lcb_scores = {}
        if self.tracer:
            self._trace_selection_path.append(self.tracer.get_node_id(node))
        while node.children:
            if self._can_expand(node):
                return node
            viable = [c for c in node.children
                      if c.plan.cost() < self._best_cost]
            if not viable:
                return node
            if self.tracer:
                for c in viable:
                    cid = self.tracer.get_node_id(c)
                    self._trace_lcb_scores[cid] = self._lcb(c, node)
            node = min(viable, key=lambda c: self._lcb(c, node))
            if self.tracer:
                self._trace_selection_path.append(self.tracer.get_node_id(node))
        return node

    def _lcb(self, child: MCTSNode, parent: MCTSNode) -> float:
        if child.visit_count == 0:
            return float("-inf")
        alpha = self.cfg.get("lcb_best_weight", 0.7)
        exploit = alpha * child.best_cost + (1 - alpha) * child.avg_cost
        explore = self.cfg.ucb_exploration * math.sqrt(
            math.log(parent.visit_count + 1) / child.visit_count)
        return exploit - explore

    def _can_expand(self, node: MCTSNode) -> bool:
        k = math.ceil(
            self.cfg.pw_C * (max(node.visit_count, 1) ** self.cfg.pw_alpha))
        return len(node.children) < k

    # -- Expansion --

    def _expand(self, node: MCTSNode, grid_env: GridEnv, state: State):
        """
        One-sentence summary: This is the expansion step of MCTS — it picks the highest-cost open edge in the current plan, generates candidate subgoals to resolve it, and creates a new child node from the first untried candidate that successfully applies.

        Step-by-step walkthrough:

        Pick an open edge (lines 478–480): Selects the open edge with the highest relaxed cost — this is the "most expensive gap" heuristic for where to decompose next.

        Get candidates (line 482): Calls _get_candidates to propose possible subgoal decompositions for the chosen edge (bottleneck/support pairs that could close it).

        Sort by score (line 486): Candidates are sorted ascending by score (lower = better), so the best candidate is tried first.

        Skip already-tried actions (lines 490–497): Each candidate is hashed by its key fields (edge, bottleneck pos, support pos, helper color, parent support pos). If this node already has a child with that action, skip it.

        Apply the subgoal (lines 499–505): Copies the plan, then calls _apply_subgoal to insert the subgoal's nodes (bottleneck, support, leaves) into the DAG. If it fails (e.g., unreachable positions), tries the next candidate.

        Eagerly close edges (lines 508–509): After applying, iterates all remaining open edges and tries to close any that are now directly reachable (no further decomposition needed).

        Create child node (lines 511–517): Wraps the new plan in an MCTSNode, links it to the parent, registers it with the tracer if present, and checks if the plan is now complete (no open edges, valid cost).

        Returns None if no candidate succeeded — this node is effectively a dead end for expansion.

        Non-obvious behavior: Only one child is created per _expand call (returns on first success), which is standard for MCTS progressive widening. The tried set ensures repeated expansions on the same node explore different candidates.
        """
        self._trace_target_edge = None
        self._trace_candidates_count = 0
        self._trace_candidate_positions = []
        self._trace_expansion_closed = []
        plan = node.plan
        open_edges = plan.open_edges()
        if not open_edges:
            return None

        parent_id, child_id = max(
            open_edges, key=lambda e: plan.g.edges[e]["cost"] or 0)
        self._trace_target_edge = (parent_id, child_id)

        candidates = _get_candidates(plan, parent_id, state, grid_env)
        if not candidates:
            return None

        candidates.sort(key=lambda x: x[1])
        self._trace_candidates_count = len(candidates)
        seen_pos = set()
        unique_positions = []
        for sg, _score, _psp in candidates:
            p = sg.bottleneck.position
            if p not in seen_pos:
                seen_pos.add(p)
                unique_positions.append(list(p))
        self._trace_candidate_positions = unique_positions

        tried = {c.action for c in node.children}
        for subgoal, _score, parent_sp in candidates:
            action_key = (parent_id, child_id,
                          subgoal.bottleneck.position,
                          subgoal.support.position,
                          subgoal.helper.color,
                          parent_sp)
            if action_key in tried:
                continue

            child_plan = _copy_plan(plan)
            result = _apply_subgoal(
                child_plan, parent_id, child_id,
                subgoal, grid_env, self._cnt,
                parent_support_pos=parent_sp)
            if result is None:
                continue

            # Close only provably optimal edges
            expansion_closed = []
            for pid, cid in list(child_plan.open_edges()):
                if _should_close_edge(child_plan, pid, cid, grid_env, state):
                    _try_close_edge(child_plan, pid, cid, grid_env)
                    expansion_closed.append((pid, cid))
            self._trace_expansion_closed = expansion_closed

            # Prune if optimistic cost can't beat best
            if child_plan.cost() >= self._best_cost:
                break # TODO this was continue but the relaxed cost are optimistic so we could be able to break here

            child_node = MCTSNode(
                plan=child_plan, parent=node, action=action_key)
            self._node_count += 1
            node.children.append(child_node)
            if self.tracer:
                self.tracer.register_node(child_node, parent=node)
            self._check_complete(child_plan, grid_env)

            # Try speculative completion for upper bound discovery
            if not child_plan.is_complete():
                completed, _ = _try_complete_plan(
                    child_plan, grid_env)
                if completed is not None:
                    self._eval_complete(completed, grid_env)

            return child_node

        return None

    # -- Rollout (random greedy completion) --

    def _rollout(self, node: MCTSNode, grid_env: GridEnv, state: State):
        self._rollout_count += 1
        self._trace_rollout_steps = []
        self._trace_rollout_completed = False
        import random
        plan = _copy_plan(node.plan)

        for step_i in range(self.cfg.max_rollout_depth):
            # Prune if optimistic cost can't beat best
            if plan.cost() >= self._best_cost:
                return plan.cost()

            # Close all directly closeable edges (rollout is disposable)
            closed_edges = []
            for pid, cid in list(plan.open_edges()):
                if _try_close_edge(plan, pid, cid, grid_env):
                    closed_edges.append((pid, cid))

            open_edges = plan.open_edges()
            if not open_edges:
                if self.tracer and closed_edges:
                    self._trace_rollout_steps.append({
                        "step": step_i, "type": "close_only",
                        "closed": [list(e) for e in closed_edges],
                    })
                break

            # Pick random open edge
            parent_id, child_id = random.choice(open_edges)

            candidates = _get_candidates(plan, parent_id, state, grid_env)
            if not candidates:
                if self.tracer:
                    self._trace_rollout_steps.append({
                        "step": step_i, "type": "no_candidates",
                        "edge": [parent_id, child_id],
                        "closed": [list(e) for e in closed_edges],
                    })
                return self.cfg.penalty_cost

            # Pick random candidate from top half
            candidates.sort(key=lambda x: x[1])
            pick_from = max(1, len(candidates) // 2)
            subgoal, _, parent_sp = random.choice(candidates[:pick_from])

            result = _apply_subgoal(
                plan, parent_id, child_id,
                subgoal, grid_env, self._cnt,
                parent_support_pos=parent_sp)

            if self.tracer:
                step_data = {
                    "step": step_i, "type": "subgoal",
                    "edge": [parent_id, child_id],
                    "bn_pos": list(subgoal.bottleneck.position),
                    "sp_pos": list(subgoal.support.position),
                    "helper_color": subgoal.helper.color,
                    "applied": result is not None,
                    "closed": [list(e) for e in closed_edges],
                    "candidates_count": len(candidates),
                }
                self._trace_rollout_steps.append(step_data)

            if result is None:
                continue

        # Close remaining closeable edges
        final_closed = []
        for pid, cid in list(plan.open_edges()):
            if _try_close_edge(plan, pid, cid, grid_env):
                final_closed.append((pid, cid))
        if self.tracer and final_closed:
            self._trace_rollout_steps.append({
                "step": "final", "type": "close_only",
                "closed": [list(e) for e in final_closed],
            })

        if plan.is_complete():
            self._trace_rollout_completed = True
            self._check_complete(plan, grid_env)
            cost = evaluate_plan(plan, grid_env)
            if cost is not None:
                return cost

        return plan.cost()

    # -- Backpropagation --

    def _backprop(self, node: MCTSNode, cost: float):
        while node is not None:
            node.visit_count += 1
            node.total_cost += cost
            node.best_cost = min(node.best_cost, cost)
            if self.tracer:
                self.tracer.update_node(node)
            node = node.parent

    # -- Helpers --

    def _eval_complete(self, plan: PartialPlan, grid_env: GridEnv) -> float:
        cost = evaluate_plan(plan, grid_env)
        if cost is not None:
            if cost < self._best_cost:
                self._best_cost = cost
                self._best_plan = _copy_plan(plan)
            self._all_plans.append(PlanEntry(
                plan=_copy_plan(plan),
                cost=cost,
                stats=PlanStats(
                    wall_time=time.monotonic() - self._start_time,
                    iteration=self._iteration,
                    node_count=self._node_count,
                    rollout_count=self._rollout_count,
                )
            ))
            return cost
        return self.cfg.penalty_cost

    def _check_complete(self, plan: PartialPlan, grid_env: GridEnv):
        if plan.is_complete():
            self._eval_complete(plan, grid_env)

    def _fallback(self, root: MCTSNode, grid_env: GridEnv, state: State):
        best_node = root
        stack = [root]
        while stack:
            n = stack.pop()
            if n.visit_count > 0 and n.best_cost < best_node.best_cost:
                best_node = n
            stack.extend(n.children)

        plan = _copy_plan(best_node.plan)
        for _ in range(self.cfg.max_rollout_depth):
            open_edges = plan.open_edges()
            if not open_edges:
                break
            parent_id, child_id = max(
                open_edges, key=lambda e: plan.g.edges[e]["cost"] or 0)
            if _try_close_edge(plan, parent_id, child_id, grid_env):
                continue
            candidates = _get_candidates(plan, parent_id, state, grid_env)
            if not candidates:
                break
            best_sub, _, best_sp = min(candidates, key=lambda x: x[1])
            result = _apply_subgoal(
                plan, parent_id, child_id,
                best_sub, grid_env, self._cnt,
                parent_support_pos=best_sp)
            if result is None:
                break

        # Close any remaining closeable edges
        for parent_id, child_id in list(plan.open_edges()):
            _try_close_edge(plan, parent_id, child_id, grid_env)

        return plan
